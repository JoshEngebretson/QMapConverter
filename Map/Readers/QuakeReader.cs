    ...
    public class QuakeReader : AbstractMapReader
    {
        ...
        public override BrushMap ReadMap(string inputFile, Dictionary<string, string> settings)
        {
            ...
            if (settings.ContainsKey("scale"))
                scale = QMapConverter.Util.MathExt.FromString(settings["scale"]);
            ...
            {
                ++lineNum;
                float percentage = ((float)lineNum) / ((float)fileData.Count);
                percentage *= 100;
                ...
                else
                {
                    ...
                    else if (currentEntity != null)
                    {
                        ...
                        else
                        {
                            ...
                            List<Match> terms = regex.Matches(shortLine).Cast<Match>().ToList();

                            // Insert KvP properties, if a Value is non-existent, insert 'true' as it's likely a flag
                            if (terms.Count == 2)
                                currentEntity.Properties[terms[0].Value.Replace("\"", "")] = terms[1].Value.Replace("\"", "");
                            else
                                currentEntity.Properties[terms[0].Value.Replace("\"", "")] = "true";
                        }
                    }
                }
            }
